...
For each command, samdebug selects distinct dynamic GDB, TCL, and telnet ports
on `127.0.0.1`, starts the pinned OpenOCD with `cmsis_dap_backend hid`, SWD,
`target/at91sam4sXX.cfg`, the configured probe serial, and the configured adapter
speed. Because OpenOCD cannot inherit pre-bound listener sockets, a bind
collision causes a bounded retry with three newly selected ports; exhausted
collisions fail as `LOCAL_PORT_UNAVAILABLE`. The process is time-bounded,
cancellation-aware, and owned until it has exited; normal and error paths issue
OpenOCD shutdown and leave no server or probe lock behind.
...
An erase or flash timeout also uses exit code 6 and explicitly reports that the
destructive operation may be partial and the target firmware state is unknown.
